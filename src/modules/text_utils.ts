...
export function getChapterDesignInstruction(lang) {
    if (lang === 'Korean') {
        return `In section 5, keep each chapter readable as a story outline first. Every chapter entry must include a concrete chapter title, then preserve the narrative labels "내용:" and "핵심 포인트:" with concrete story content under them. A safe format is "제 N장: 장 제목" followed by "내용:" and "핵심 포인트:". Add the compact generation-control fields after those labels, not instead of them: chapter_function (primary and optional secondary), start_scene, end_state or end_hook, must_include, must_not_include or not_this_chapter, chapter_keywords, reveal_or_knowledge_step, and intensity scores from 0-10 for external_threat, relationship_drama, mystery, combat, and comedy when relevant. Vary adjacent chapter functions so the outline does not repeat the same conflict engine, accusation loop, rescue shape, power reveal, relationship beat, or ending hook unless it returns with a new consequence.`;
    }
    if (lang === 'Japanese') {
        return `In section 5, keep each chapter readable as a story outline first. Every chapter entry must include a concrete chapter title, then preserve narrative labels equivalent to "内容:" and "重要ポイント:" with concrete story content under them. A safe format is "第 N 章: chapter title" followed by "内容:" and "重要ポイント:". Add the compact generation-control fields after those labels, not instead of them: chapter_function (primary and optional secondary), start_scene, end_state or end_hook, must_include, must_not_include or not_this_chapter, chapter_keywords, reveal_or_knowledge_step, and intensity scores from 0-10 for external_threat, relationship_drama, mystery, combat, and comedy when relevant. Vary adjacent chapter functions so the outline does not repeat the same conflict engine, accusation loop, rescue shape, power reveal, relationship beat, or ending hook unless it returns with a new consequence.`;
    }
    return `In section 5, keep each chapter readable as a story outline first. Every chapter entry must include a concrete chapter title, then preserve the narrative labels "Content:" and "Key Points:" with concrete story content under them. A safe format is "Chapter N: chapter title" followed by "Content:" and "Key Points:". Add the compact generation-control fields after those labels, not instead of them: chapter_function (primary and optional secondary), start_scene, end_state or end_hook, must_include, must_not_include or not_this_chapter, chapter_keywords, reveal_or_knowledge_step, and intensity scores from 0-10 for external_threat, relationship_drama, mystery, combat, and comedy when relevant. Vary adjacent chapter functions so the outline does not repeat the same conflict engine, accusation loop, rescue shape, power reveal, relationship beat, or ending hook unless it returns with a new consequence.`;
}
...
function parseChapterNumberToken(raw) {
    if (!raw) return NaN;
    const token = raw.trim().replace(/[０-９]/g, ch =>
...
export function splitPlotIntoChapters(plotText) {
    const normalized = String(plotText || '');
    ...
    for (let i = 0; i < matches.length; i++) {
        ...
        map[num] = normalized.slice(start, end).trim();
    }
    return map;
}
