...
export function getChapterDesignInstruction(lang) {
    ...
    if (lang === 'Korean') {
        return `In section 5, keep each chapter readable as a story outline first. Every chapter entry must preserve the narrative labels ${narrativeLabels} with concrete story content under them. Add compact generation-control fields after those labels, not instead of them. Required fields for every chapter: ${requiredFields}. Use pov_primary for the main viewpoint holder and pov_secondary for any additional viewpoint or focus character; write "none" if there is no secondary POV. Write emotional_delta from the perspective of pov_primary, and if pov_secondary is not "none", split it by character name inside the same field instead of blending emotions. Use opening_state_or_scene for the chapter's starting condition plus the first visible scene beat. Use outcome_state for the concrete result, cost, and closing state caused by the chapter's main action. Use knowledge_shift for the reveal, clue, misunderstanding, and who knows/suspects/does not know by chapter end. Use plant_for_later for a specific clue, image, object, phrase, injury, rule, or behavior that should pay off several chapters later; write "none" only when no delayed setup is useful. Write intensity as one compact object or line with 0-10 values for external_threat, relationship_drama, mystery, combat, and comedy. Add optional genre-specific fields only when useful: ${optionalGenreFields}. Use not_this_chapter for hard chapter-level exclusions and do_not_resolve_yet for conflicts, mysteries, relationships, reveals, or emotional shifts that must not be settled too early. Make each control field describe a concrete story state change, choice, consequence, knowledge boundary, cost, delayed setup, or emotional movement rather than an abstract mood. Vary adjacent chapter functions so the outline does not repeat the same conflict engine, accusation loop, rescue shape, power reveal, relationship beat, foreshadowing device, or ending hook unless it returns with a new consequence.`;
    }
    if (lang === 'Japanese') {
